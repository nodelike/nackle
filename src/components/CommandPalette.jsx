...
const CommandPalette = memo(function CommandPalette({ open, onClose, collections, todos, onSelectCollection, onSelectTodo }) {
    ...
    const results = useMemo(() => {
        ...
        // collections
        const matchedCols = q ? collections.filter((c) => c.name.toLowerCase().includes(q)) : collections;
        ...
        const matchedTodos = q ? pendingTodos.filter((t) => t.text.toLowerCase().includes(q)) : pendingTodos;
        ...
        <div
            ...
            <div style={S.modal}>
                ...
                <div style={S.list} ref={listRef}>
                    {results.length === 0 && <div style={S.empty}>No results</div>}

                    {colResults.length > 0 && <div style={S.sectionLabel}>Collections</div>}
                    {colResults.map((item) => {
                        flatIndex++;
                        const idx = flatIndex;
                        return (
                    ...
                    {todoResults.length > 0 && <div style={S.sectionLabel}>Tasks</div>}
