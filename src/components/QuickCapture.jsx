...
const S = {
  ...
  modal: {
    width: 480, background: "rgb(var(--bg-rgb) / 0.85)",
  ...
  stepLabel: {
    ...
    letterSpacing: 1, textTransform: "uppercase", padding: "12px 16px 0",
  },
  inputRow: {
  ...
  item: {
    display: "flex", alignItems: "center", gap: 10,
    padding: "7px 16px", cursor: "pointer", transition: "background 0.06s",
    fontSize: 13, color: "var(--fg)", opacity: 0.85,
  },
  ...
  selectedBadge: {
    ...
    padding: "2px 8px", border: "1px solid var(--border)", color: "var(--fg-muted)",
    flexShrink: 0, display: "flex", alignItems: "center", gap: 6,
  },
  badgeX: {
    cursor: "pointer", color: "var(--fg-muted)", fontSize: 12, lineHeight: 1,
  },
};

const QuickCapture = memo(function QuickCapture({
  open,
  onClose,
  collections,
  onCreateTodo,
  ...
  const showCreate = step === "collection" && query.trim() &&
    !collections.some((c) => c.name.toLowerCase() === query.trim().toLowerCase());

  const items = step === "collection"
    ...
    <div style={S.overlay} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={S.modal}>
        <div style={S.stepLabel}>
          {step === "collection" ? "1 \u2014 pick collection" : "2 \u2014 enter task"}
        </div>

        <div style={S.inputRow}>
          {step === "todo" && selectedCollection && (
            <span style={S.selectedBadge}>
              {selectedCollection.isNew ? "+ " : ""}{selectedCollection.name}
              <span style={S.badgeX} onClick={goBackToCollection}>&times;</span>
            </span>
          )}
          <input
            ref={inputRef}
