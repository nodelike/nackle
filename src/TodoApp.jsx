...
export default function TodoApp() {
  ...
  const addTodoToCollection = useCallback(async (collectionId, text) => {
    const id = generateId();
    const newItem = {
      id, text, done: 0, collection_id: collectionId,
      created_at: Date.now(), sort_order: 0, archived_at: null,
    };
  ...
  const archiveTodo = useCallback(async (id) => {
    ...
    try { await window.db.archiveTodo(id); } catch (e) { console.error(e); }
  }, []);
  ...
  const restoreTodo = useCallback(async (id) => {
    ...
    try { await window.db.restoreTodo(id); } catch (e) { console.error(e); }
  }, []);

  const deleteForever = useCallback(async (id) => {
    setArchived((prev) => prev.filter((t) => t.id !== id));
    try { await window.db.deleteForever(id); } catch (e) { console.error(e); }
  }, []);

  const emptyArchive = useCallback(async () => {
    setArchived([]);
    setArchivedCollections([]);
    try { await window.db.emptyArchive(); } catch (e) { console.error(e); }
  }, []);

  const updateTodo = useCallback(async (id, text) => {
    setTodos((prev) => prev.map((t) => (t.id === id ? { ...t, text } : t)));
    try { await window.db.updateTodo({ id, text }); } catch (e) { console.error(e); }
  }, []);

  // ── Collection CRUD ──
  const createCollectionRaw = useCallback(async (name) => {
    ...
    try { await window.db.addCollection({ id, name, icon: "\u25C6", color }); } catch (e) { console.error(e); }
    return id;
  }, [collections.length]);
  ...
  const renameCollection = useCallback(async (id, name) => {
    ...
    try { await window.db.updateCollection({ id, name }); } catch (e) { console.error(e); }
  }, []);

  const archiveCollection = useCallback(async (id) => {
    if (id === "inbox") return;
    ...
    try { await window.db.archiveCollection(id); } catch (e) { console.error(e); }
  }, [collections]);

  const restoreCollection = useCallback(async (id) => {
    ...
    try { await window.db.restoreCollection(id); } catch (e) { console.error(e); }
  }, [archivedCollections]);
  ...
  const deleteCollection = useCallback(async (id) => {
    ...
    setActiveCollection((prev) => (prev === id ? "inbox" : prev));
    try { await window.db.deleteCollection(id); } catch (e) { console.error(e); }
  }, []);

  // ── Drag & Drop ──
  const handleReorderDrop = useCallback(async (targetIndex, listItems) => {
    ...
    setDropIndex(null);
    setDraggedTodo(null);
    try { await window.db.reorderTodos(updates); } catch (e) { console.error(e); }
  }, [draggedTodo]);
  ...
  const clearCompleted = useCallback(async () => {
    ...
    try { await window.db.clearCompleted(colId); } catch (e) { console.error(e); }
  }, [activeCollection]);

  // ── Quick capture handler ──
  const handleQuickCapture = useCallback(async (collection, todoText) => {
    let colId;
    if (collection.isNew) {
  ...
  const handleSaveSetting = useCallback(async (key, value) => {
    ...
    try { await window.db.saveSetting({ key, value }); } catch (e) { console.error(e); }
  }, []);
  ...
  const activeTodos = isAll
    ? sortTodos(todos)
    : sortTodos(todos.filter((t) => t.collection_id === activeCollection));
  ...
  return (
    <div className="app-shell" style={S.container}>
      ...
      <QuickCapture
        ...
        collections={collections}
        onCreateTodo={handleQuickCapture}
      />
            ...
            {pending.length === 0 && completed.length === 0 && (
              <div style={S.empty}>
                ...
                <div style={S.emptyText}>
                  {isAll ? "No tasks yet" : "All clear"}
                </div>
                <div style={S.emptyHint}>
                  {isAll ? "Press \u2318N to add a task" : "Press \u2318N to add a task"}
                </div>
              </div>
            )}
